...
describe('Game ControlV3', async function () {
  ...
  beforeEach(async () => {
    ...
    let GameControl = await ethers.getContractFactory('GameControlMockV3')
    ...
    let xDraceDistributor = await ethers.getContractFactory('xDraceDistributor')
    let xDraceDistributorInstance = await xDraceDistributor.deploy()
    xDraceVesting = await xDraceDistributorInstance.deployed()
    await xDraceVesting.initialize(xDrace.address, 0)
  ...
  async function depositNFTs(player, tokenIds) {
    ...
    await gameControl.connect(player).depositNFTsToPlayMock(tokenIds, freeTurns)

    //checking token ownership
    await checkDeposits(player, tokenIds)

    for(var i = 0; i < tokenIds.length; i++) {
      let tokenPlayingTurns = await gameControl.tokenPlayingTurns(tokenIds[i])
      expect(tokenPlayingTurns).to.be.equal(20)
    }
  }
