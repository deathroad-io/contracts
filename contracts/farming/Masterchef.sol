...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. DRACE to distribute per block.
        uint256 lastRewardBlock; // Last block number that DRACE distribution occurs.
        uint256 accDRACEPerShare; // Accumulated DRACE per share, times 1e12. See below.
    }
    // The DRACE TOKEN!
    DRACE public drace;
        ...
        returns (uint256)
    {
        ...
        return user.amount.mul(accDRACEPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accDRACEPerShare).div(1e12).sub(
                    user.rewardDebt
                );
            safeDRACETransfer(msg.sender, pending);
        }
    ...
    function safeDRACETransfer(address _to, uint256 _amount) internal {
        uint256 draceBal = drace.balanceOf(address(this));
        if (_amount > draceBal) {
            drace.transfer(_to, draceBal);
        } else {
            drace.transfer(_to, _amount);
        }
    }
