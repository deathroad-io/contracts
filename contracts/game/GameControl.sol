...
contract GameControl is Ownable, SignerRecover {
    ...
    function withdrawNFTs(uint256[] memory _tokenIds) external {
        for(uint256 i = 0; i < _tokenIds.length; i++) {
            if (tokenDeposits[_tokenIds[i]].depositor == msg.sender) {
                require(tokenLastUseTimestamp[_tokenIds[i]].timestamp.add(600) < block.timestamp, "game is in play");

                draceNFT.safeTransferFrom(address(this), msg.sender, _tokenIds[i]);
    ...
    function startGame(uint256[] memory _tokenIds, uint256 _validTimestamp, bytes32 r, bytes32 s, uint8 v) external {
        require(_validTimestamp > block.timestamp, "signature timestamp too late");
        bytes32 message = keccak256(abi.encode(msg.sender, _tokenIds, _validTimestamp));
        ...
        for(uint256 i = 0; i < _tokenIds.length; i++) {
            require(tokenDeposits[_tokenIds[i]].depositor == msg.sender, "NFT token ID not deposited yet");
            require(tokenLastUseTimestamp[_tokenIds[i]].timestamp.add(getUsePeriod(_tokenIds[i])) < block.timestamp, "NFT tokens used too frequently");
    ...
    function distributeRewards(address _recipient, uint256 _rewardAmount, uint256 _cumulativeReward, bytes32 r, bytes32 s, uint8 v) external {
        require(cumulativeRewards[_recipient].add(_rewardAmount) <= _cumulativeReward, "reward exceed cumulative rewards");
        //verify signature
        bytes32 message = keccak256(abi.encode(_recipient, _rewardAmount, _cumulativeReward));
        address signer = recoverSigner(r, s, v, message);
        require(signer == gameOperator, "distributeRewards::invalid operator");

        cumulativeRewards[_recipient] = cumulativeRewards[_recipient].add(_rewardAmount);
