    ...
    IERC721ReceiverUpgradeable
{
    ...
    function _withdrawTokens(
        ...
        if (_userInfo.draceDeposited >= _pendingToSpendDrace) {
            ...
            //burn the pending
            if (chainId == 56) {
                ERC20BurnableUpgradeable(address(drace)).burn(_pendingToSpendDrace);
            } else {
                drace.safeTransfer(address(1), _pendingToSpendDrace);
            }
        } else {
            if (chainId == 56) {
                ERC20BurnableUpgradeable(address(drace)).burn(_userInfo.draceDeposited);
            } else {
        ...
        if (_userInfo.xdraceDeposited >= _pendingToSpendxDrace) {
            ...
            ERC20BurnableUpgradeable(address(xdrace)).burn(_pendingToSpendxDrace);
        } else {
            ERC20BurnableUpgradeable(address(xdrace)).burn(_userInfo.xdraceDeposited);
        }
    ...
    function _distribute(
        ...
        IERC20Upgradeable(address(xdrace)).approve(address(xDraceVesting), _xdraceAmount);
    ...
    function _buyPlayingTurn(
        ...
        if (_usexDrace) {
            ...
            ERC20BurnableUpgradeable(address(xdrace)).burnFrom(msg.sender, xDraceNeeded); //burn xDrace immediately
        } else {
